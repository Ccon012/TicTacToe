...
function begin_play(){
	...
	document.getElementById("player1_id").value = "" + player_1 + " (X)"
	var player_2 = document.getElementById("player2_id").value
	document.getElementById("player2_id").value = "" + player_2 + " (O)"
	var move
	if(whose_move() == 1)
	{
		move = "X"
	...
	document.getElementById("turn_info").innerHTML = "Turn for: " + move
	started = 1;
	if(started == 1)
	{
...
function play() {
	...
	if(started) {
		if (table_ids.includes(input)) {
			number = table_ids.indexOf(input)
			if ((board_state[table_ids.indexOf(input)] = whose_move()) == 1) {
				table_ids[table_ids.indexOf(input)] = "X";
				document.getElementById("turn_info").innerHTML = "Turn for: " + "O".bold()
			} else {
				table_ids[table_ids.indexOf(input)] = "O"
				document.getElementById("turn_info").innerHTML = "Turn for: " + "X".bold()
			}
			document.getElementById(input).innerHTML = table_ids[number];
			toggle_move();
		} else {
			alert("Invalid move")
		}
		const [first, second, third, fourth, fifth, sixth, seventh, eighth, ninth] = board_state;
		for (var i = 0; i < 2; i++)
		{
		if((first == i && second == i && third == i) || (fourth == i && fifth == i && sixth ==i) || (seventh == i && eighth == i && ninth == i) || (first == i && fourth == i && seventh == i) || (second !== i && fifth == i && eighth == i) || (third == i && sixth == i && ninth == i) || (first == i && fifth == i && ninth == i)|| (third == i && fifth == i && seventh == i))
		{
			if (i) {
				alert("winner is X");
				reset_play();
